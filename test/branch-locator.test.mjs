...
test("bundles Fulton, DeKalb, and Fairfax public branches with valid coordinates", () => {
  ...
  assert.equal(locations.filter((location) => location.library === "fairfax").length, 22);
  assert.equal(new Set(locations.map((location) => `${location.library}:${location.code}`)).size, 79);
...
test("searches by ZIP, branch name, city, address, and system", () => {
  ...
  assert.equal(searchBranchLocations(locations, "20151")[0].code, "Chantilly Regional Library");
});
