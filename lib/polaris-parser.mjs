...
function scoreCandidate(candidate, queryTitle, queryAuthor) {
  ...
  if (actualTitle === wantedTitle) score = 0.9;
  else if (
  ...
  if (queryAuthor) {
    ...
    if (candidate.author && authorScore < 0.5) score -= 0.18;
  }
...
export function parseAvailability(html, branchName) {
  ...
  const otherBranchCount = locations.filter((location) => location !== selected && location.available > 0).length;
  ...
  return {
    status: checkedIn ? "on_shelf" : selected.total > 0 ? "checked_out" : otherBranchCount > 0 ? "elsewhere" : "not_found",
