...
function recordUrl(bibId, pos) {
  return `${CATALOG_BASE}/search/title.aspx?ctx=${CATALOG_CONTEXT}&pos=${encodeURIComponent(pos)}&cn=${encodeURIComponent(bibId)}`;
}

export async function findBestBib(session, { title, author }) {
  async function search(term, by, sort = "RELEVANCE") {
    ...
    const resultsHtml = await session.getText(`${CATALOG_BASE}/search/components/ajaxResults.aspx?${resultsParams}`);
...
export async function fetchAvailability(session, match, branchName) {
  const params = new URLSearchParams({
    fp: "1",
    level: "local",
    morelink: "0",
...
export async function checkPolarisBookAvailability(book, branchName, branchLocations = []) {
  ...
  try {
    ...
    return {
      ...
      isDigital: false,
      recordUrl: recordUrl(match.bibId, match.pos),
