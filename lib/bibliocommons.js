...
async function matchBook({ title, author }, format = "print", library) {
  let bibs = [];
...
export async function checkBookAvailability(book, branchCode, format = "print", library = "fulcolibrary") {
  ...
  let availability;
  try {
    availability = await fetchAvailability(match.bibId, branchCode, format, library);
  } catch {
    availability = {
      status: "not_found",
      callNumber: null,
