...
function gatewayBase(library) {
  return `https://gateway.bibliocommons.com/v2/libraries/${library}`;
}
...
function matchesFormat(bib, format) {
  ...
  if (format === "all") {
    return isPhysicalBook(bib) || matchesFormat(bib, "ebook") || matchesFormat(bib, "audiobook");
  }
  if (format === "ebook") {
...
async function searchBibs(library, searchType, query) {
  const url = `${gatewayBase(library)}/bibs/search?searchType=${searchType}&query=${encodeURIComponent(query)}`;
  try {
...
async function matchBook({ title, author }, format = "all", library) {
  let bibs = [];
  const clause = formatCodeClause(format);
  const scoped = [
    `title:(${blTerm(title)})`,
    author ? `author:(${blTerm(author)})` : "",
    clause,
  ].filter(Boolean).join(" ");
  bibs = await searchBibs(library, "bl", scoped);

  if (bibs.length === 0) {
    bibs = await searchBibs(library, "smart", author ? `${title} ${author}` : title);
  }
  ...
  return {
    ...
    matchedFormat: format === "all"
      ? (matchesFormat(best, "audiobook") ? "audiobook" : matchesFormat(best, "ebook") ? "ebook" : "print")
      : format,
  };
}
...
async function fetchAvailability(bibId, branchCode, format = "print", library) {
  const url = `${gatewayBase(library)}/bibs/${bibId}/availability`;
  const data = await bcFetch(url);
...
export async function checkBookAvailability(book, branchCode, format = "all", library = "fulcolibrary") {
  const input = book.input || book.title;
  const match = await matchBook(book, format, library);
  ...
  if (!match) {
    return {
      ...
      dueDate: null,
      otherBranchCount: 0,
      isDigital: format !== "print" && format !== "all",
      recordUrl: null,
      coverUrl: null,
    };
  }
  ...
  let availability;
  try {
    availability = await fetchAvailability(match.bibId, branchCode, match.matchedFormat, library);
  } catch {
    availability = {
      status: "not_found",
      callNumber: null,
      dueDate: null,
      otherBranchCount: 0,
      isDigital: match.matchedFormat !== "print",
    };
  }
  ...
  return {
    ...
    recordUrl: `https://${library}.bibliocommons.com/v2/record/${match.bibId}`,
